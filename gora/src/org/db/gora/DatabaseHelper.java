...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    static DbColumnInfo getColumnInfo(FieldData fieldData) {
        ...
        switch (fieldData.dataType) {
            ...
            default:
                Log.w(Settings.TAG, String.format("Unsupported data type: \"%s\"", fieldData.dataType.toString()));
                columnInfo.columnType = DbColumnType.TEXT;
                break;
        }
        ...
        for (TableData tableData: sqlSchema.tableMap.values()) {
            ...
            while (fieldCursor.moveToNext()) {
                ...
                if (pkColumn == null) {
                    ...
                    if (!fType.startsWith("INT")) {
                        Log.e(Settings.TAG, String.format("Primary key %s in table %s is expected to be integer", pkColumn, tableInfo.tableName));
                    }
                } else {
            ...
            if (dbColumns.size() > 0) { //table exists
                if (tableInfo.pkName.equalsIgnoreCase(pkColumn)) {
                    Log.e(Settings.TAG, String.format("Primary key field name for table %s are different: Database: %s,  Schema: %s", tableInfo.tableName, pkColumn, tableInfo.pkName));
                }
            ...
            if (tableData.indice != null) {
                ...
                for (DbIndexInfo indexInfo: dbIndice) {
                    if (indexInfo.columns == null) {
                        indexInfo.columns = new ArrayList<String>();
                    } else {
...
//                      int rank = indexCursor.getInt(1);
                ...
                for (IndexData indexData: tableData.indice) {
                    ...
                    if (foundIndex == null) {
                        ...
                        foundIndex.columns = new ArrayList<String>();
